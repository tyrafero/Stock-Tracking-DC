...
const TransferDetail: React.FC = () => {
  ...
  const dispatchMutation = useMutation({
    mutationFn: () => stockAPI.dispatchTransfer(Number(id)),
    onSuccess: () => {
      notifications.show({
        title: 'Transfer Dispatched',
        message: 'Transfer has been dispatched successfully',
        color: 'blue',
      });
      queryClient.invalidateQueries({ queryKey: ['transfer', id] });
      queryClient.invalidateQueries({ queryKey: ['transfers'] });
  ...
  const handleDispatch = () => {
    modals.openConfirmModal({
      ...
      children: (
        <Text>
          Are you sure you want to dispatch this transfer? This confirms that the
          stock has been physically sent to the destination.
        </Text>
      ),
      labels: { confirm: 'Dispatch', cancel: 'Cancel' },
  ...
  const canApprove = transfer.status === 'pending' && hasPermission('approve_transfer');
  const canDispatch = transfer.status === 'in_transit' && hasPermission('dispatch_transfer');
  const canCollect = transfer.status === 'awaiting_collection' && hasPermission('collect_transfer');
  const canCancel = ['pending', 'in_transit'].includes(transfer.status) && hasPermission('cancel_transfer');
