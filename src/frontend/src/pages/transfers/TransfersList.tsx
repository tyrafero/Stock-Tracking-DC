...
const TransfersList: React.FC = () => {
  ...
  const columns: DataTableColumn<StockTransfer>[] = useMemo(() => [
    ...
    },
    {
      ...
      width: 150,
      render: (transfer) => (
        <Text size="sm">{transfer.requested_by?.username || 'Unknown'}</Text>
      ),
    },
    {
      accessor: 'created_at',
    ...
    },
    {
      ...
      render: (transfer) => (
        <Group gap="xs" justify="center">
          ...
          {transfer.status === 'pending' && hasPermission('transfers.approve_stocktransfer') && (
          ...
          {transfer.status === 'approved' && hasPermission('transfers.complete_stocktransfer') && (
          ...
          {transfer.status === 'awaiting_collection' && hasPermission('transfers.complete_stocktransfer') && (
