...
const StockDetail: React.FC = () => {
  ...
  const {
    ...
    queryFn: () => stockAPI.getReservations({ stock_id: id }),
    enabled: !!id,
    staleTime: 30000,
  });
  ...
  const {
    ...
    queryKey: ['committed-stock', id],
    queryFn: () => stockAPI.getCommittedStock({ stock_id: id }),
    enabled: !!id,
    staleTime: 30000,
  });
  ...
  const commitMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['stock', id] });
      queryClient.invalidateQueries({ queryKey: ['committed-stock', id] });
      closeCommit();
      resetCommitForm();
    },
    onError: (error: any) => {
        ...
        <Grid>
                ...
                {historyLoading ? (
                  ...
                  <Box>
                    <Table>
                      ...
                      <Table.Tbody>
                        {stockHistory.slice(0, 10).map((history: StockHistory) => (
                    ...
                    {stockHistory.length > 10 && (
                      <Text c="dimmed" size="xs" mt="xs" align="center">
                        Showing 10 of {stockHistory.length} records
                      </Text>
                    )}
                  </Box>
                ) : (
          ...
          <Grid.Col span={{ base: 12, md: 4 }}>
            <Stack gap="md">
              ...
              {(stock.location || stock.aisle || stock.warehouse_name) && (
                <Card shadow="sm" padding="lg" radius="md" withBorder>
                  <Group mb="md">
                  ...
                  <Stack gap="xs">
                    {stock.warehouse_name && (
                      <Text size="sm">Warehouse: {stock.warehouse_name}</Text>
                    )}
                    {stock.location && (
                      <Text size="sm">Location: {stock.location}</Text>
                    )}
                    {stock.aisle && (
                      <Text size="sm">Aisle: {stock.aisle}</Text>
                    )}
                  </Stack>
                </Card>
              )}
